...
trait HasAssets
{
    ...
    public static function css($css = null, $minify = true)
    {
        ...
        $css = array_filter(array_unique($css));
            ...
            if (!$css = static::getMinifiedCss()) {
                $css = array_merge(static::$css, static::baseCss2());
            }
    ...
    public static function baseCss2($css = null, $minify = true)
    {
        static::ignoreMinify($css, $minify);

        if (!is_null($css)) {
            return static::$baseCss2 = $css;
        }

        $skin = config('admin.skin', 'skin-blue-light');

        array_unshift(static::$baseCss2, "vendor/laravel-admin/AdminLTE/dist/css/skins/{$skin}.min.css");

        return static::$baseCss2;
